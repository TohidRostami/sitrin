    ...
    <main>
      <section className="relative overflow-hidden border-b border-border">
        <div
          ...
          style={{ background: "radial-gradient(90% 100% at 80% 0%, #640E0E 0%, transparent 60%)" }}
        />
        <div className="relative mx-auto max-w-[1000px] px-4 py-14 sm:px-6 md:px-10 md:py-24">
          <div className="mb-4.5 text-xs font-extrabold tracking-[0.12em] text-brand-hover">{about.eyebrow}</div>
          <h1 className="mb-6 text-[32px] font-black leading-[1.1] tracking-tight text-balance md:text-[56px] lg:text-[64px]">
            {about.title}
          </h1>
          <p className="max-w-[62ch] text-[15px] leading-8 text-muted md:text-lg">{about.body}</p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1180px] px-4 py-10 sm:px-6 md:px-10 md:py-14">
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          {about.stats.map((s) => (
            <div key={s.label} className="rounded-[20px] border border-border bg-surface p-6">
              <div className="text-[28px] font-black tracking-tight text-brand md:text-[36px]">{s.value}</div>
      ...
      <section className="mx-auto max-w-[1180px] px-4 pb-12 sm:px-6 md:px-10 md:pb-20">
        <div className="grid grid-cols-1 items-center gap-8 lg:grid-cols-2 lg:gap-12">
          <div className="relative aspect-[4/3] overflow-hidden rounded-[24px] border border-border">
            <ProductImage src={heroImages[1]?.url ?? heroImages[0]?.url} alt="فروشگاه سیترین" />
          </div>
